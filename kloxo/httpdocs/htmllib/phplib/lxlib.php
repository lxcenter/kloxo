...
{
    if(is_unlimited($priv)) return false;
    if(is_unlimited($used)) return true;
    if(isOn($priv)) return false;
    if(isOn($used)) return true;
...
function initProgramlib($ctype = null)
{
    ...
    if ($var >= 2 ) dprint("initProgramlib called twice \n <br> ");
...
function initSession($object, $ssl_param, $consuming_parent)
{
    ...
    if ($consuming_parent) $ckstart .= "-consumed";
...
function get_classvar_description($class, $var = null)
{
    ...
    if (!$ret) return null;
    $ret['help'] = $ret[2];
    if (cse($dvar, "_o") || cse($dvar, "_l")) return $ret;
    if (!$g_language_desc) return $ret;
