...
class HtmlLib
{
    ...
    function __construct()
    {
        ...
        if(isset($tmp['frm_o_o']) && $tmp['frm_o_o'])
        {
            ...
            {
                if(isset($k)) self::checkForScript($k);
                if(isset($v['class'])) self::checkForScript($v['class']);
                if(isset($v['nname'])) self::checkForScript($v['nname']);
            }
        }
        ...
        foreach($tmp as $key => $value)
        {
            if(char_search_a($key, "_aaa_"))
            {
                ...
                else $hvar[$arvar][$arkey] = $arval; #[FIXME] Same behaviour?
            }
            else
            {
                if(!is_array($value))
                {
                    ...
                    else $hvar[$key] = $value; #[FIXME] Same behaviour?
    ...
    function printTabButtonOld($key, $url)
    {
        ...
        ?>
        <td >
        <table cellspacing=0 cellpadding=0  <?=$idstring?> <?=$borderbottom ?> valign=bottom> <tr valign=bottom>
        <?php
        if($check)
